...
export default function PetClient({ pet: p, isNew }: { pet: PetFull; isNew: boolean }) {
  ...
  const ownerEmail = (p.owners as any)?.email as string | null
    ...
    <>
      ...
      <div className="min-h-screen flex items-start justify-center py-8 px-4">
        <div className="w-full max-w-sm bg-white rounded-[28px] overflow-hidden shadow-[0_24px_64px_rgba(11,31,59,0.16)]">
          ...
          {/* Vlasnik */}
          {(p.owners?.name || p.owners?.phone || ownerEmail) && (
            <Sec title={t('prof_owner')}>
              {p.owners?.name && <Row l={t('row_name')} v={(p.owners as any).name} />}
              {p.owners?.phone && <Row l={t('row_phone')} v={(p.owners as any).phone} c="teal" />}
