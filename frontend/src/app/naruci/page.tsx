    ...
    <div className="min-h-screen bg-[#F4F7FA]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-extrabold text-navy tracking-tight mb-3">{t('nav_order')}</h1>
          <p className="text-gray-500 font-medium">{t('products_sub')}</p>
        </div>

        {/* Coming soon — ghost cards + frosted overlay */}
        <div className="relative max-w-3xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pointer-events-none select-none" aria-hidden="true">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-64 rounded-3xl bg-gradient-to-b from-navy/8 to-navy/4 border border-[#E2EAF0]" />
            ))}
          </div>
