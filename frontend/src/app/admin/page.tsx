...
export default function AdminPage() {
  ...
  const login = () => {
    if (pin === (process.env.NEXT_PUBLIC_ADMIN_PIN || 'petcode2025')) { setAuth(true); load() }
  ...
  const load = async () => {
    ...
    const codes = qrR.data || []
    const ords = ordersR.data || []
    setQr(codes); setOrders(ords); setPets(petsR.data||[])
    setStats({
      total: codes.length,
  ...
  const generateQr = async () => {
    setLoading(true)
    const codes = Array.from({length:gen},()=>({ code:'PC-'+Math.random().toString(36).substring(2,8).toUpperCase() }))
    await sb.from('qr_codes').insert(codes)
    await load()
  }
  ...
  const exportCsv = () => {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download='petcode-qr.csv'; a.click()
  }
  ...
  const STATUS_COLORS: Record<string,string> = {
    nova:'bg-orange-50 text-orange-600', potvrdjena:'bg-blue-50 text-blue-600',
    poslata:'bg-purple-50 text-purple-600', isporucena:'bg-green-50 text-green-600',
  }
  ...
  return (
    <div className="min-h-screen bg-[#f0fffe]">
      <nav className="bg-navy px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={()=>setAuth(false)} className="text-white/40 text-xs font-bold hover:text-white">Odjavi se</button>
      </nav>
              ...
              <div key={o.id} className="card py-3 px-4">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  ...
                  <div className="flex flex-col gap-1.5 items-end">
                    ...
                    <select className="text-xs border border-[#e2f0ef] rounded-xl px-2 py-1 font-semibold text-gray-500 bg-white"
        ...
        {tab==='qr' && (
          <div className="space-y-4">
            <div className="card flex items-end gap-3 flex-wrap">
              ...
              <button onClick={generateQr} disabled={loading} className="btn-navy">+ Generiši {gen}</button>
              ...
              {filteredQr.map(q=>(
                <div key={q.id} className="card flex items-center justify-between py-2.5 px-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {q.status==='active'&&<button onClick={()=>sb.from('qr_codes').update({status:'disabled'}).eq('id',q.id).then(load)} className="text-xs text-red-400 font-bold">Deaktiviraj</button>}
                    {q.status==='disabled'&&<button onClick={()=>sb.from('qr_codes').update({status:'unused'}).eq('id',q.id).then(load)} className="text-xs text-green-500 font-bold">Aktiviraj</button>}
                  </div>
                </div>
              ))}
            </div>
            ...
            {filteredPets.map(p=>(
              <div key={p.id} className="card flex items-center gap-3 py-2.5 px-4">
                <div className="w-10 h-10 rounded-xl bg-teal/10 flex items-center justify-center text-xl overflow-hidden flex-shrink-0">
                  {p.photo_url?<img src={p.photo_url} className="w-full h-full object-cover rounded-xl"/>:'🐾'}
                </div>
